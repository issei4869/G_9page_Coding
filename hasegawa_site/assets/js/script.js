...
jQuery(function ($) {
  ...
  $(function () {
    ...
    if (pageHash) {
      var scrollToElement = $(pageHash); // `data-id`ではなく、`id`属性をターゲットに
      ...
      $('body').fadeIn(1500, function () {
        setTimeout(function () {
          ...
          var locationOffset = scrollToElement.offset().top; // ターゲット要素の位置を取得
          var navigationBarHeight = $('.p-header').innerHeight(); // ヘッダーの高さを取得
  ...
  var href = location.href.split("#")[0].split("?")[0]; // ハッシュやクエリを除外

  var links = document.querySelectorAll(".p-pc-nav > li > a");
  for (var i = 0; i < links.length; i++) {
    var linkHref = links[i].href.split("#")[0].split("?")[0]; // リンクのハッシュやクエリを除外
    console.log(linkHref);
    if (linkHref == href && links[i].querySelector("p").textContent != "採用情報") {
      var pTag = links[i].querySelector("p"); // aタグ内のpタグを取得
      if (pTag) {
        // pタグが存在するか確認
  ...
  $(window).on('scroll pageshow', function (event) {
    /* ページスクロール、キャッシュ復元時 */
    var scroll = $(this).scrollTop(); /* 現在のスクロール量を測定 */
    var windowHeight = $(window).height(); /* ウィンドウの高さを測定 */
  ...
  var service_slideLength = document.querySelectorAll('.js-newequipment-swiper .swiper-slide').length;
  $(window).resize(function () {
    service_arrow();
  });
  service_arrow();
  ...
  var btn = document.querySelector('.p-top-recruit__btn');
  var img = document.querySelector('.p-top-recruit__img img');
  if (btn && img) {
    // ボタンにホバーしたときに画像を拡大
  ...
  /////////////////////// 矢印が一方通行で戻ってくる ///////////////////////
  var arrowLinks = document.querySelectorAll('.p-sub-equipment__item');
  ...
  /////////////////////// チェックボックスカウント ///////////////////////
  var checkboxes = document.querySelectorAll('.p-about-recruit__checkbox input[type="checkbox"]');
  var checkedCountElem = document.querySelector('.p-about-recruit__checked-count');
  var totalCountElem = document.querySelector('.p-about-recruit__total-count');

  // チェックされた数を数えて表示する関数
  function updateCheckedCount() {
    var checkedCount = document.querySelectorAll('.p-about-recruit__checkbox input:checked').length;
  ...
  sections.forEach(function (section, index) {
    var pinTarget = section.querySelector("[data-section-inner]");
    ScrollTrigger.create({
      trigger: section,
      start: "center center",
  ...
  function resizeCard() {
    var aboutItems = document.querySelectorAll(".p-about-item");
    var aboutItemsHeight = [];
    ...
    var maxHeight = Math.max.apply(Math, aboutItemsHeight);
    aboutItems.forEach(function (item) {
  ...
  $(window).scroll(function () {
    ...
    if (newEquipmentHeading.length > 0) {
      // ターゲットが存在するか確認
